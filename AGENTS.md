...
- Depth is **monocular relative** only: UI `0=Near` / `1=Far` is within-image ordering, not metres or cross-image scale. Paper/README wording: relative depth ordering, apparent size / relative size–distance features, image-relative near/far categorization — never metric distance. `estimate_depth_scale_m` is a calibration hook that returns `None` in shipped ARTPS.
