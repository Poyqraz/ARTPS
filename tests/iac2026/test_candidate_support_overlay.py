...
def test_open_corners_not_full_rectangle():
    ...
    assert not np.array_equal(out[20, 20:60], np.tile(BRACKET_COLOR, (40, 1)))


def test_cc_contour_drawn_and_anchor_at_map_max():
    ...
    out = draw_candidate_support_overlay(rgb, dets, combined)
    edge = out[18, 30]
    assert int(edge[2]) > 80 or int(edge[1]) > 80, edge.tolist()
    # dark-edged white anchor near map max
    ay, ax = 28, 32
