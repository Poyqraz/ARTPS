"""
ARTPS - Çalışan Basit Autoencoder Modeli

Bu modül, Mars kaya görüntülerini analiz etmek için kullanılan
basit ve çalışan bir Convolutional Autoencoder modelini içerir.
"""
...
class ReshapeLayer(nn.Module):
    """
    Tensor'ı yeniden şekillendirmek için özel katman
    """
...
class WorkingAutoencoder(nn.Module):
    """
    Çalışan basit Convolutional Autoencoder modeli
    ...
    def __init__(self, input_channels: int = 3, latent_dim: int = 16384):
        ...
        """
        super(WorkingAutoencoder, self).__init__()
        ...
        self.decoder = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            ReshapeLayer(256, 8, 8),  # Linear çıkışını reshape et
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # Decoder - latent'i doğrudan decoder'a ver, decoder kendi reshape'ini yapacak
...
class AutoencoderTrainer:
    ...
    def __init__(self, model: WorkingAutoencoder, device: str = 'cuda'):
        """
        Args:
...
def visualize_reconstruction(model: WorkingAutoencoder, 
                           image: torch.Tensor, 
    ...
    # Görüntüyü model ile aynı cihaza taşı
...
def main():
    ...
    if os.path.exists(data_dir):
        ...
        print("Model kaydedildi: results/working_autoencoder_model.pth")
        
        # Örnek görselleştirme
        if len(dataset) > 0:
            sample_image = dataset[0]
            visualize_reconstruction(model, sample_image, "results/working_reconstruction_example.png")
