...
Convolutional Autoencoder Modeli

Bu modül, Mars kaya görüntülerini analiz etmek için kullanılan
Convolutional Autoencoder modelini içerir. Model, görüntüleri alıp
yeniden oluşturur ve yeniden oluşturma hatasını hesaplar.
Bu hata, anomali puanı olarak kullanılır.
"""
...
class ConvolutionalAutoencoder(nn.Module):
    ...
    Encoder: Görüntüyü latent space'e sıkıştırır
    Decoder: Latent representation'dan orijinal görüntüyü yeniden oluşturur
    """
    ...
    def __init__(self, input_channels: int = 3, latent_dim: int = 128):
        ...
        self.decoder = nn.Sequential(
            ...
            # İlk dekonvolüsyon bloğu: 256 -> 128
            nn.ConvTranspose2d(256, 128, kernel_size=4, stride=2, padding=1),
            nn.BatchNorm2d(128),
            nn.ReLU(inplace=True),
            
            # İkinci dekonvolüsyon bloğu: 128 -> 64
            nn.ConvTranspose2d(128, 64, kernel_size=4, stride=2, padding=1),
            nn.BatchNorm2d(64),
            nn.ReLU(inplace=True),
            
            # Üçüncü dekonvolüsyon bloğu: 64 -> 32
            nn.ConvTranspose2d(64, 32, kernel_size=4, stride=2, padding=1),
            nn.BatchNorm2d(32),
            nn.ReLU(inplace=True),
            
            # Dördüncü dekonvolüsyon bloğu: 32 -> input_channels
...
class AutoencoderTrainer:
    ...
    def __init__(self, model: ConvolutionalAutoencoder, device: str = 'cuda'):
        """
        Args:
...
def visualize_reconstruction(model: ConvolutionalAutoencoder, 
                           image: torch.Tensor, 
...
def main():
    ...
    # Model oluştur
    model = ConvolutionalAutoencoder(input_channels=input_channels, latent_dim=latent_dim)
    ...
    if os.path.exists(data_dir):
        ...
        # Modeli kaydet
        trainer.save_model("results/autoencoder_model.pth")
        print("Model kaydedildi: results/autoencoder_model.pth")
        
        # Örnek görselleştirme
        if len(dataset) > 0:
            sample_image = dataset[0]
            visualize_reconstruction(model, sample_image, "results/reconstruction_example.png")
