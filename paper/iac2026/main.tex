...
% Canonical accepted ARTPS paper code (title block + footer).
\renewcommand{\IACpapercode}{IAC-26,A3,IP,109,x109221}
\renewcommand{\IACcorresponding}{* Corresponding author: poyrazbaydemir@gmail.com}

\begin{document}

\IACmaketitle
{ARTPS: Operationally Safe Target Prioritization via Depth-Enhanced Hybrid Anomaly Detection}
{Poyraz Baydemir\textsuperscript{a,*}}
{\textsuperscript{a} Faculty of Technology, Sel\c{c}uk University, Konya, T\"urkiye}
{%
...
ARTPS combines image and relative-depth cues in a multi-cue scoring architecture that
supports entropy-weighted adaptive fusion, ranks valid candidates after image-level scoring
with a curiosity score and soft similarity penalty, and uses a Priority Buffer for
second-chance reconsideration of high-anomaly targets down-weighted by diversity policy.
Operational gates address rover-body and boundary-shadow false positives, protect objects
inside shadows, and apply an apparent-size policy conditioned on image-relative depth.
...
% twocolumn: \newpage is a column break; start [12] at the top of the right column.
\makeatletter
\let\ARTPSoldbibitem\bibitem
\def\bibitem{\@ifnextchar[\ARTPSbibopt\ARTPSbibplain}
\def\ARTPSbibopt[#1]#2{%
  \def\ARTPSthis{#2}%
  \def\ARTPStarget{bell2021mastcamz}%
  \ifx\ARTPSthis\ARTPStarget\newpage\fi
  \ARTPSoldbibitem[#1]{#2}}
\def\ARTPSbibplain#1{\ARTPSoldbibitem{#1}}
\makeatother
