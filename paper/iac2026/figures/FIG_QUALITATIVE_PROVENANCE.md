...
| Overlay visualization | `candidate_support_v2` (open-corner ROI + low-alpha support footprint + combined-map anchor) |
| `support_geometry_source` | proposal hysteresis/CC contour persisted as visualization-only metadata; drawn as low-alpha footprint, not a polyline silhouette; no new map threshold |
