...
| C02 | Entropy-weighted dynamic fusion | Methods / future ablation | fusion code in `app.py` / `artps_detection_core.py`; pinned eval TBD | T_detect / Fig fusion | `implemented` | Entropy rule is the architectural specification (historical MS). Current detection core used for frozen independent-eval scoring is fixed-weight multi-cue fusion. Entropy-vs-fixed performance delta still `planned`. |
