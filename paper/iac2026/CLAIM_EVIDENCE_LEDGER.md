...
| IND_EVAL_V1 | Current reproducible image-binary detection eval (protocol pin) | Independent bench | `INDEPENDENT_EVALUATION_PROTOCOL.md`, `INDEPENDENT_EVAL_V1_ANNOTATION_GUIDE.md`, `INDEPENDENT_EVAL_V1_DATASET_PLAN.md`, `reproduction/iac2026/INDEPENDENT_EVAL_V1.yaml` (runtime lock via `independent_eval_contract.py`), `configs/independent_evaluation.example.yaml` / `.synthetic.yaml`, header-only `manifests/independent_eval_v1.template.csv` | T_detect_indep (planned) | `protocol_defined_pending_data` | Execution contract live (lock SHA, provenance, split builder refuse pending ratios); not a reproduction of C05/C06 accepted numbers; no real run / no dataset rows yet |
