...
| IND_EVAL_V1 | Current reproducible image-binary detection eval (protocol pin) | Independent bench | `INDEPENDENT_EVALUATION_PROTOCOL.md`, `reproduction/iac2026/INDEPENDENT_EVAL_V1.yaml`, `configs/independent_evaluation.example.yaml`, header-only `manifests/independent_eval_v1.template.csv` | T_detect_indep (planned) | `protocol_defined_pending_data` | Not a reproduction of C05/C06 accepted numbers; no real run until SHA-pinned labeled manifest exists |
